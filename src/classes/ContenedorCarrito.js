...
class ContenedorCarritos{
    ...
    async agregarProducto(cid,body){
        try{
           ...
           let result = carritos.map(carrito=>{
                if(carrito.id===cid){
                        body = Object.assign(body)
                        body = Object.assign({id:cid,...body})
                        return body;
                }else{
                    return carrito;
                }})
                try{
                    await fs.promises.writeFile(carritosURL,JSON.stringify(result,null,2));
